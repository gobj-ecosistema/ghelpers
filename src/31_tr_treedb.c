...
PUBLIC json_t *_treedb_create_topic_cols_desc(void)
{
    ...
    json_array_append_new(
        topic_cols_desc,
        json_pack("{s:s, s:s, s:i, s:s, s:[s,s,s,s,s,s,s,s,s,s,s,s,s,s,s,s,s,s,s,s], s:[s,s,s]}",
            "id", "flag",
            "header", "Flag",
            "fillspace", 14,
            "type", "array",
...
)
{
    ...
    if(webix) {
        if(!kw_has_key(jchild, "value")) {
            const char *id = kw_get_str(jchild, "id", "", KW_REQUIRED);
            const char *name = kw_get_str(jchild, "name", "", 0);
            if(empty_string(name)) {
                json_object_set_new(jchild, "value", json_string(id));
            } else {
                json_object_set_new(jchild, "value", json_string(name));
            }
        }
    }
    json_decref(jchild_);
